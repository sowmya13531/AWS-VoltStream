...
export default function SmartControl() {
  ...
  const toggleDevice = async (id, currentState) => {
    try {
      ...
      setDevices(devices.map(d => d.id === id ? { ...d, is_on: !currentState } : d));
      
      await axios.patch(`${API_BASE}/devices/${id}`, {
        is_on: !currentState
      });
    } catch (error) {
  ...
  if (loading) {
    return <div className="text-slate-400 p-8">Loading Smart Devices...</div>;
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
      <header className="mb-8 flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-100">Smart Control</h1>
          <p className="text-slate-400 mt-2">Manage your connected appliances</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-lg px-4 py-2 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
          <span className="text-sm text-slate-300">{devices.filter(d => d.is_on).length} Active</span>
        </div>
      </header>
          ...
          return (
            ...
            >
              <div className="flex justify-between items-start mb-6">
                <div className={`p-3 rounded-xl ${device.is_on ? 'bg-blue-500/20 text-blue-400' : 'bg-slate-800 text-slate-500'}`}>
                  <Icon className="w-6 h-6" />
                </div>
                <button
                  onClick={() => toggleDevice(device.id, device.is_on)}
                  className={`w-14 h-8 rounded-full transition-colors relative flex items-center px-1 ${
                    device.is_on ? 'bg-blue-500' : 'bg-slate-700'
                  }`}
                >
